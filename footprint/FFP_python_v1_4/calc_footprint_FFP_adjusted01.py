...
class FFP:
    ...
    def output(self, zm=None, z0=None, umean=None, h=None, ol=None, sigmav=None, ustar=None,
        ...
        # Check passed values
        if zm <= 0.: self.raise_ffp_exception(2)
        if z0 is not None and umean is None and z0 <= 0.: self.raise_ffp_exception(3)
        if h <= 10.: self.raise_ffp_exception(4)
        if zm > h: self.raise_ffp_exception(5)
        if z0 is not None and umean is None and zm <= 12.5*z0:
            if rslayer is 1: self.raise_ffp_exception(6)
            else: self.raise_ffp_exception(12)
        if float(zm)/ol <= -15.5: self.raise_ffp_exception(7)
        if sigmav <= 0: self.raise_ffp_exception(8)
        if ustar <= 0.1: self.raise_ffp_exception(9)
        if wind_dir is not None:
            if wind_dir> 360 or wind_dir < 0: self.raise_ffp_exception(10)
        if nx < 600: self.raise_ffp_exception(11)

        # Resolve ambiguity if both z0 and umean are passed (defaults to using z0)
        if None not in [z0, umean]: self.raise_ffp_exception(13)
        ...
        if rs is not None:
            ...
            if isinstance(rs, numbers.Number):
                ...
                rs = [rs]
            if not isinstance(rs, list): self.raise_ffp_exception(14)
        ...
        dy = dx
        if rs is not None:
            clevs = self.get_contour_levels(f_2d, dx, dy, rs)
            frs = [item[2] for item in clevs]
            xrs = []
            yrs = []
            for ix, fr in enumerate(frs):
                xr,yr = self.get_contour_vertices(x_2d, y_2d, f_2d, fr)
                if xr is None:
                    frs[ix] = None
                xrs.append(xr)
                yrs.append(yr)
            ...
            if crop:
                ...
                xrs = []
                yrs = []
                xrs,yrs = self.get_contour_vertices(x_2d, y_2d, f_2d, clevs[0][2])
        ...
        # Plot footprint
        if fig:
            fig_out,ax = self.plot_footprint(x_2d=x_2d, y_2d=y_2d, fs=f_2d,
