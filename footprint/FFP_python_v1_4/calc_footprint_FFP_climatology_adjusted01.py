...
class FFP_climatology:
    ...
    def output(self, zm=None, z0=None, umean=None, h=None, ol=None, sigmav=None, ustar=None,
        ...
        if any(len(lst) != ts_len for lst in [sigmav, wind_dir, h, ol]):
            ...
            self.raise_ffp_exception(11, verbosity)

        # Special treatment for zm, which is allowed to have length 1 for any
        # length >= 1 of all other parameters
        if all(val is None for val in zm): self.raise_ffp_exception(12, verbosity)
        if len(zm) == 1:
            self.raise_ffp_exception(17, verbosity)
        ...
        if not all(val is None for val in z0):
            self.raise_ffp_exception(13, verbosity)
            ...
            self.raise_ffp_exception(14, verbosity)
            z0 = [None for i in range(ts_len)]
        else:
            self.raise_ffp_exception(15, verbosity)
        ...
        if rs is not None:
            ...
            if isinstance(rs, numbers.Number):
                ...
                rs = [rs]
            if not isinstance(rs, list): self.raise_ffp_exception(18, verbosity)
            ...
            if np.max(rs) > 0.9:
                self.raise_ffp_exception(19, verbosity)
        ...
        for ix, (ustar, sigmav, h, ol, wind_dir, zm, z0, umean) \
            ...
            valids[ix] = self.check_ffp_inputs(ustar, sigmav, h, ol, wind_dir, zm, z0, umean, rslayer, verbosity)

            # If inputs are not valid, skip current footprint
            if not valids[ix]:
                self.raise_ffp_exception(16, verbosity)
            else:
                #===========================================================================
        ...
        if n==0:
            ...
            # Plot footprint
            if fig:
                fig_out,ax = self.plot_footprint(x_2d=x_2d, y_2d=y_2d, fs=fclim_2d,
    ...
    def check_ffp_inputs(self, ustar, sigmav, h, ol, wind_dir, zm, z0, umean, rslayer, verbosity):
        ...
        if h <= 10.:
            ...
            return False
        if zm > h :
            self.raise_ffp_exception(5, verbosity)
            return False
        if z0 is not None and umean is None and zm <= 12.5*z0:
            if rslayer is 1:
                self.raise_ffp_exception(6, verbosity)
            else:
                self.raise_ffp_exception(20, verbosity)
                return False
        if float(zm)/ol <= -15.5:
            self.raise_ffp_exception(7, verbosity)
            return False
        if sigmav <= 0:
            self.raise_ffp_exception(8, verbosity)
            return False
        if ustar <= 0.1:
            self.raise_ffp_exception(9, verbosity)
            return False
        if wind_dir > 360:
            self.raise_ffp_exception(10, verbosity)
            return False
        if wind_dir < 0:
            self.raise_ffp_exception(10, verbosity)
            return False
    ...
    #===============================================================================
    def plot_footprint(self, x_2d, y_2d, fs, clevs=None, show_heatmap=True, normalize=None,
